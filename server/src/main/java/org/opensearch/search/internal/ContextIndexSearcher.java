...
public class ContextIndexSearcher extends IndexSearcher implements Releasable {
    ...
    /**
     * Performs the postCollection on the {@link BucketCollector} which is used to collect documents for Aggregation. This is needed
     * for concurrent segment search as there are AssertingCodecs in lucene which validates that the DocValues created for a field is always
     * used by the same thread for a request. In concurrent segment search case, the DocValues gets created on different threads for
     * different segments (or slices). Later in reduce phase when postCollection happens that is performed on the separate thread which
     * is from search threadpool and not from slice threadpool. So two different threads performs the access on the DocValues causing the
     * AssertingCodec to fail. From functionality perspective, there is no issue as DocValues for each segment is always accessed by a
     * single thread at a time but those threads may be different (e.g. slice thread during collection and then search thread during reduce)
     * @param collector
     * @throws IOException
     */
